...
function showMetropolisModal() {
  ...
  modal.className = 'metropolis-modal';
  modal.innerHTML = `
    <h3>Select Metropolis</h3>
    <div class="metropolis-buttons">
      ${metropolises.map(metropolis => `
        <button 
          class="metropolis-button" 
          onclick="filterLocators('${metropolis}', 'metropolis')"
        >
          ${metropolis}
        </button>
      `).join('')}
    </div>
    <button onclick="closeMetropolisModal()" class="close-modal-button">Close</button>
  `;
  document.body.appendChild(modal);
}

function closeMetropolisModal() {
  const modal = document.querySelector('.metropolis-modal');
  if (modal) {
